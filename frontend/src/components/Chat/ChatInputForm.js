 ...
 * @description file containing ChatInputForm component
 */
 ...
 * @param {Function} props.toggleUsersModal - Function to toggle the user modal (can be activeUsers for native OR... privateUsers for private)
 *
...
const ChatInputForm = ({
  ...
  const handleMentionClickWrapper = (displayName) => {
    const value = inputRef.current;
    const newValue =
      value.substring(0, value.lastIndexOf('@')) + `@${displayName} `;
    handleInputChange({ target: { value: newValue } });
    setShowDropdown(false);
  };
