 ...
 * @description file containing Message component
 */
 ...
 * @param {string} props.chatId - NATIVE: The ID of the video associated with the message. PRIVATE: unique id
 ...
 * @param {Function} props.removeMessageDB - Function to handle the removal of a message from database.
 * @param {Function} props.addReactionDB - Function to add reaction
 * @param {Function} props.muteUserDB - Function to mute user
 *
...
const Message = ({
  ...
  if (user !== null) {
    ...
    messageClasses = message.text.includes(`@${user.displayName}`)
      ? 'text-current bg-primary rounded-xl p-1'
      : 'text-current'; // Adds highlighting if the message mentions the user
  }
    ...
    if (muteDuration) {
      try {
        await muteUserDB(
          message.chatId,
          message.authorUid,
          parseInt(muteDuration)
        );
        alert(
          `User ${message.authorName} has been muted for ${muteDuration} minutes.`
        );
      } catch (error) {
      ...
      <div className="flex-grow">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center space-x-2">
            ...
            <span
              className={`text-sm font-bold ${isUserMessage ? 'text-primary' : 'text-accent '}`}
            >
              {message.authorName}
            </span>
            <span className="text-xs font-light text-current">
          ...
          <div className="relative">
            ...
            <span
              className="three-dots cursor-pointer text-current"
            ...
            {showDropdown && (
              <div
                className="bg-primary absolute z-10 mt-1 rounded-lg shadow-lg w-48 right-0"
              ...
              >
                <ul className="py-2 text-sm text-current rounded-xl">
                  {isUserMessage && (
                    <li>
                      ...
                      <p
                        ...
                        className="rounded-xl block p-2 m-2 hover:bg-accent dark:hover:bg-gray-600 dark:hover:text-white"
                      >
                        Remove message
                      </p>
                    </li>
                  ...
                  <li>
                    <p
                      onClick={() => handleSeeAccountInfo(message.authorUid)}
                      className="rounded-xl block p-2 m-2 hover:bg-accent dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      See account info
                    </p>
                  ...
                  <li>
                    <p
                      onClick={() => console.log('Add to chat')}
                      className="rounded-xl block p-2 m-2 hover:bg-accent dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      Add to chat
                    </p>
          ...
          {message.reactions && (
            <div className="flex space-x-2 mt-2">
              {Object.entries(message.reactions).map(([reaction, count]) => (
                <div
                  key={reaction}
                  className="flex items-center space-x-1 btn btn-sm"
                  onClick={() => {
