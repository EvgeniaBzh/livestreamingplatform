 ...
 * @description file containing NativeChat component
 */
...
/**
 * NOTE: This component is currently commented out and not in use. (to be replced with ClubverseChat.js)
 *
 ...
 *
 * @returns {JSX.Element} The rendered NativeChat component.
 */
const NativeChat = ({ videoId, activeUsers, toggleActiveUsersModal }) => {
    ...
    if (value.includes('@')) {
      ...
      if (mentionPart) {
        const filteredUsers = activeUsers.filter((user) =>
          user.displayName.toLowerCase().includes(mentionPart)
        );
  ...
  return (
    <div className="relative flex flex-col w-full h-full bg-neutral rounded-xl">
      <div className="px-8 py-2 rounded-lg shadow-lg text-center items-center">
        ...
        <span className="w-full whitespace-nowrap overflow-hidden text-ellipsis block text-sm">
          {videoTitle}
        </span>
      </div>
